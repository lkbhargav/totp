...
pub fn add_account() -> Result<(), Box<dyn Error>> {
    println!("account: ");
    let mut account = String::new();
    io::stdin()
        .read_line(&mut account)
        .expect("unable to read line");
    if account.trim_end().is_empty() {
...
pub fn delete_account(account: &str) -> Result<(), Box<dyn Error>> {
    println!("Deleting account {}", account);
    accounting::delete_account(&account.to_owned())?;
    Ok(())
}

pub fn generate_token(account: &str) -> Result<(), Box<dyn Error>> {
